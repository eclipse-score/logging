...
# Client A must keep running effectively forever, so that a freeze can be
# observed for as long as the test needs -- passed as a generic args string
# to the constructor, not a hardcoded iterations parameter, so this same
# class works for any binary/flag combination elsewhere in the suite.
_CLIENT_A_ARGS = "--it 1000000"
# ---------------------------------------------------------------------------
# Helpers
# ---------------------------------------------------------------------------
...
def test_datarouter_stuck_on_client_sigstop(target_fixture):
    ...
    with RemoteBinaryExecution(
        target_fixture, _APP_PATH, _LOGGING_APP, args=_CLIENT_A_ARGS
    ) as client_a:
        ...
        #           Teardown (SIGCONT + SIGTERM) runs in a finally block so it
        #           always executes even if an assertion fails.
        # ------------------------------------------------------------------
        try:
            ...
            client_a.send_signal("CONT")
            client_a.send_signal("TERM")
