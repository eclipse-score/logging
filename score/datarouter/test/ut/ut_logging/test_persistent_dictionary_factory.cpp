...
#include "score/aracoreinitwrapper/mocklib/aracoreinitializer_mock.hpp"
...
class PersistentDictionaryFactoryTest : public ::testing::Test
{
  protected:
    void SetUp() override
    {
        ara_core_initializer_mock_ = std::make_unique<score::aracoreinitwrapper::AraCoreInitializerMock>();
    }

    void TearDown() override
    {
        ara_core_initializer_mock_.reset();
    }

    std::unique_ptr<score::aracoreinitwrapper::AraCoreInitializerMock> ara_core_initializer_mock_;
};

TEST_F(PersistentDictionaryFactoryTest, CreatePersistentDictionaryShallReturnAraPerPersistentDictionary)
{
    bool no_adaptive_runtime = false;
    auto pd = AraPerPersistentDictionaryFactory::Create(no_adaptive_runtime);
    EXPECT_TRUE(IsPersistentDictionaryOfType<AraPerPersistentDictionary>(pd));
}

TEST_F(PersistentDictionaryFactoryTest, CreatePersistentDictionaryShallReturnStubPersistentDictionary)
{
