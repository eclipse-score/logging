...
SCENARIOS = [
    {"name": "quota_disabled_500", "enabled": False, "limit_kbps": 500},
    {"name": "quota_enabled_500", "enabled": True, "limit_kbps": 500},
...
def test_quota_exceed(target, datarouter_manager, dlt_capture):
    ...
    assert results["quota_enabled_500"] <= results["quota_disabled_500"], (
        f"500 enabled ({results['quota_enabled_500']}) should be <= disabled "
        f"({results['quota_disabled_500']})"
    )
    assert results["quota_enabled_15"] < results["quota_enabled_500"], (
        f"15 ({results['quota_enabled_15']}) should be < 500 "
        f"({results['quota_enabled_500']})"
    )
